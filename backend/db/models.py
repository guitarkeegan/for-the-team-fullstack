...
class Team(Base):
    ...
    home_games = relationship("GameSchedule", foreign_keys="[GameSchedule.home_id]", back_populates="home_team")
    away_games = relationship("GameSchedule", foreign_keys="[GameSchedule.away_id]", back_populates="away_team")
...
class Roster(Base):
    ...
    contract_type = Column(Enum(ContractTypeEnum), nullable=False)  # Changed to Enum
...
class Lineup(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('team_id', 'player_id', 'game_id', 'period', 'time_in', 'lineup_num', name='uq_lineup')
    )
