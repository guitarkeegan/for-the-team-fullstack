...
const GameStintsPage: React.FC = () => {
  ...
  const [stintsCache, setStintsCache] = useState<Map<string, Stint[]>>(new Map());
  ...
  const fetchPlayerStints = useCallback(async (gameId: string, teamId: string, playerId: number) => {
    ...
    try {
      const response = await fetch(`/api/lineups/wide?game_id=${gameId}&team_id=${teamId}&player_id=${playerId}`);
      const data: { lineups: Lineup[] } = await response.json();
